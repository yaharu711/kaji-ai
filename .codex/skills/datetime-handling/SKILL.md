...
- アプリ内でJST固定の扱いと、DB/ORMがUTC正規化する挙動が混在し、解釈ズレが起きやすい。
- APIの送受信でTZなし日時が混ざると、フロント/バックでローカル解釈が分かれやすい。
- `timestamp` はTZ情報を保持しないため、混在すると後から追えない。

### 良い状態

- DBはUTC保存で統一し、APIはTZ付きISOのみを扱う。
- 表示はフロントがローカルTZに変換するだけに責務分離されている。
- どの層で何のタイムゾーンを扱うかが明確で、判断コストが低い。

### そのための方針

- DB保存はUTC。
- APIレスポンスはUTC ISO（`.toISOString()` / `dayjs().toISOString()`）。
- APIリクエストはISO + offsetを受け取り、UTCに正規化して保存。
- 表示はフロントでローカルTZに変換（`new Date(utcIso)` や `dayjs` で）。
